...
n <- 6 * number_of_contests

x <- tibble(pIB = map_int(1:n, ~ roll_two_dice()),
            pJM = map_int(1:n, ~ roll_two_dice())) %>%
  mutate(IB_craps = pIB %in% c(7,11)) %>%
  mutate(JM_craps = pJM %in% c(7,11)) %>%
  mutate(winner = "Tie") %>%
